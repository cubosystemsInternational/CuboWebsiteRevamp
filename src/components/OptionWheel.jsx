...
const OptionWheel = forwardRef(({
  ...
  // Shared by item clicks and the imperative `select` handle below: moves
  // the wheel to `index` by the shortest path (wrapping through the loop
  // point when `loop` is on) rather than jumping straight to its raw value.
  ...
  // Lets a controlling parent (which has no other way to command this
  // otherwise-uncontrolled wheel — there is no `selected` prop) move the
  // wheel the same way a click would: animated via the existing rAF loop,
  // firing the same onChange. Used only for external sync (e.g. the
  // Technology panel's prev/next arrows); the wheel's own interactions
  // never call this.
  useImperativeHandle(ref, () => ({ select: selectIndex }), [selectIndex]);
